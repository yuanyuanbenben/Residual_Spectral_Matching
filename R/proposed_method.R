# Proposed methods for matrix completion with spectral correction:
# 1. Matrix_factor_new_func: Nonconvex matrix factorization with spectral correction
# 2. Nuclear_opt_new_func: Convex nuclear norm optimization with spectral correction

# ============ Nonconvex Matrix Factorization with Spectral Correction ============
# Matrix_factor_new_func: Nonconvex matrix factorization with spectral correction
# Inputs:
#   X1, X2: vectors of row and column indices of observed entries
#   Y: vector of observed values
#   n: number of observations (length of Y)
#   p, q: matrix dimensions (rows, columns)
#   lambda1: regularization parameter for Frobenius norm penalty
#   lambda2: weight for spectral correction term
#   r: target rank
#   beta: bandwidth parameter for kernel smoothing
#   sigma: noise standard deviation (if known, otherwise estimated)
#   itertime: maximum number of iterations
#   tor: convergence tolerance
#   init: whether to use initialization
#   M_input: initialization matrix (if init=TRUE)
#   step_size: learning rate
#   penalty: type of regularization ('F_norm', 'Balanced', or 'None')
# Output: Estimated matrix of dimensions p x q

Matrix_factor_new_func <- function(X1,X2,Y,n,p,q,lambda1,lambda2,r,beta=0.2,sigma=0,itertime=30000,tor=1e-4,init=FALSE,M_input=FALSE,step_size=10,penalty='F_norm'){
  ...
  for (iter in 1:itertime){
    ...
    if (as.integer(iter/20)*20==iter){
      print(iter)
      # M_new = U%*%t(V)
      # obj_value <- sum((M_new-M_old)**2) / p / q
      # print(obj_value)
      # if (abs(obj_value) < tor){
      #   break
      # }
      # M_old = M_new
...
# ============ Convex Nuclear Norm Optimization with Spectral Correction ============
# Nuclear_opt_new_func: Convex nuclear norm optimization with spectral correction
# Inputs:
#   X1, X2: vectors of row and column indices of observed entries
#   Y: vector of observed values
#   n_t: number of observations (length of Y)
#   p, q: matrix dimensions (rows, columns)
#   lambda: regularization parameter (threshold for singular values)
#   gamma: step size for gradient update
#   beta: bandwidth parameter for kernel smoothing
#   itertime: maximum number of iterations
...
Nuclear_opt_new_func <- function(X1,X2,Y,n_t,p,q,lambda,gamma,beta=0.2,itertime=30000,constant=TRUE,tor=1e-4,init=FALSE,M_input=FALSE){
