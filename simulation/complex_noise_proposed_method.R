Matrix_factor_new_complex_func <- function(X1,X2,Y,n,p,q,lambda1,lambda2,r,beta=0.2,itertime=30000,tor=1e-4,init=FALSE,M_input=FALSE,step_size=10,penalty='F_norm'){
  ...
  index = TRUE
  M_0 = U%*%t(V)
  fit_noise = matrix(0,p,q)
  for (i in 1:n){
    fit_noise[X1[i],X2[i]] = Y[i] - M_0[X1[i],X2[i]]
  }
  fit_sp = svd(fit_noise)$d
  fit_sp[1:(2*r)] = fit_sp[2*r+1]
  fit_sp[(q-2*r+1):q] = fit_sp[q-2*r]
  R1 = round(min(p,q)/3)
  R2 = min(p,q) - round(min(p,q)/3)
  sum_lambda = sum(fit_sp[R1:R2])
  sigma_hat = 0
  weight = rep(1,q)
  ...
  for (iter in 1:itertime){
    ...
    sigma_hat =  sum(svd_residual$d[R1:R2]) / sum_lambda *0.9
    ...
    # random_per = rnorm(q)*sigma_sp
    # print(svd_residual$d-(diag_H)*sigma_hat)
    bias = (svd_residual$d-(fit_sp)*sigma_hat) * weight
