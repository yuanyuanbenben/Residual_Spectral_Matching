# Matrix denoising simulation with varying noise levels
# This script compares three matrix denoising methods across a range of noise standard deviations:
...
#
# The simulation sweeps over noise levels sigma = 0.005, 0.010, ..., 0.5 (100 steps)
# For each noise level, it computes average error metrics over 100 Monte Carlo trials.
#
# Input: None (parameters hardcoded)
# Output: CSV files containing error metrics for each method, plus a plot
...
# Error measurement function using propack.svd for efficiency
...
# Main Monte Carlo loop: sweep over noise levels (multi = 1,...,100)
loss_total <- foreach(multi=1:100,.verbose=TRUE,.combine = rbind) %dopar% {
  ...
  for (i in 1:100) {
    ...
    # Method 1: Matrix factorization (hard thresholding)
    if (s == 1){
    ...
    # Method 2: Nuclear norm soft-thresholding
    ...
    # Method 3: RMT optimal shrinkage
