...
loss_total <- foreach(tuning = c(1300,1400,1500,1700,2000,2300,2500,2800,3000),.verbose=TRUE,.combine = rbind) %dopar% {
  ...
  # write.csv(loss_our,paste("output/case3_convex_loss_our1",tuning,".csv",sep="_"))
  loss_baseline1 = data.frame(loss_return[9:16])
  write.csv(loss_baseline1,paste("output/case3_convex_loss_baseline1",tuning,".csv",sep="_"))
  loss_return
}

# loss_our = data.frame(loss_total[,1:8])
# write.csv(loss_our,"output/case3_convex_loss_our1.csv")
# loss_baseline1 = data.frame(loss_total[,9:16])
# write.csv(loss_baseline1,"output/case3_convex_loss_baseline1.csv")
