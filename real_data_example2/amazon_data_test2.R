...
loss_total <- foreach(s = 1:10,.verbose=TRUE,.combine = rbind) %dopar% {
  ...
  write.csv(loss_our,paste("output/case2_nonconvex_loss_our1",s,".csv",sep="_"))
  loss_baseline1 = data.frame(loss_return[9:16])
  write.csv(loss_baseline1,paste("output/case2_nonconvex_loss_baseline1",s,".csv",sep="_"))
  loss_return
}

loss_our = data.frame(loss_total[,1:8])
write.csv(loss_our,"output/case2_nonconvex_loss_our1.csv")
loss_baseline1 = data.frame(loss_total[,9:16])
write.csv(loss_baseline1,"output/case2_nonconvex_loss_baseline1.csv")
