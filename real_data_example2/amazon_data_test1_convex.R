...
# baseline compare
# loss_total <- foreach(tuning = c(30,60,100,130,160,200,230,260,300,330,360,400,430,460,500,530,560,600,630,660),.verbose=TRUE,.combine = rbind) %dopar% {
loss_total <- foreach(tuning = c(1300,1400,1500,1700,2000,2300,2500,2800,3000),.verbose=TRUE,.combine = rbind) %dopar% {
  ...
  write.csv(loss_baseline1,paste("output/case1_convex_loss_baseline1",tuning,".csv",sep="_"))
  loss_return
}

# loss_our = data.frame(loss_total[,1:8])
# write.csv(loss_our,"output/case1_convex_loss_our1.csv")
# loss_baseline1 = data.frame(loss_total[,9:16])
# write.csv(loss_baseline1,"output/case1_convex_loss_baseline1.csv")
